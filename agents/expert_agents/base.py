...
class BaseExpertAgent(ABC):
    ...
    def retrieve_knowledge_bundle(
        ...
        exact_hits = exact_search_in_categories(ltm, question, search_types)
        if exact_hits:
            hit_questions = [
                str(row.get("question", "")).replace("\n", " ")[:100]
                for row in exact_hits[:3]
            ]
            logger.info(
                "[TRACE] expert=%s question_type=%s search_types=%s retrieval=exact hits=%s hit_questions=%s",
                self.agent_name,
                self.question_type,
                search_types,
                len(exact_hits),
        ...
        logger.info(
            "[TRACE] expert=%s question_type=%s search_types=%s retrieval=exact_miss hits=0",
            self.agent_name,
            self.question_type,
            search_types,
        )
