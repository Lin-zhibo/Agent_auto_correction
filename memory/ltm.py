...
def update_ltm(
    ...
    # 去重策略：在同一父节点（同一分类路径）下，如果已经存在“问题文本完全相同”的条目，
    # 则直接使用该条目，覆盖为最新答案（认为新答案更优）。
    question_stripped = question.strip()
    for existing_key, existing_node in parent.items():
        if not isinstance(existing_node, dict):
            continue
        content = (existing_node.get("content") or "").strip()
        if not content:
            continue
        first_line = content.split("\n", 1)[0].strip()
        if not first_line.startswith("问："):
            continue
        existing_question = first_line[2:].strip()
        if existing_question == question_stripped:
            logger.info(
                "LTM 去重：检测到同一父路径下已存在相同问题，使用新答案覆盖旧答案。父路径=%s, 条目键=%s",
                topic_path[:-1],
                existing_key,
            )
            existing_node["content"] = f"问：{question}\n答：{answer}"
            existing_node["question_type"] = qt
            return qt
