# -*- coding: utf-8 -*-
"""
Evaluation script based on Judge LLM.

- Keeps real-time accuracy / elapsed time / token output
- Keeps final answer and reference answer output
- Writes logs to log/app.log
"""
...
def _build_csv_path(dataset_path: Path, model_name: str, csv_output: str | None = None) -> Path:
    ...
    if not csv_output:
        return Path("output") / "sys" / default_name
...
def _build_log_filename(dataset_path: Path, model_name: str) -> str:
    dataset_name = _sanitize_filename_part(dataset_path.stem or "dataset")
    safe_model_name = _get_model_name_for_filename(model_name)
    return f"{safe_model_name}_{dataset_name}_sys.log"
...
def main() -> None:
    ...
    parser.add_argument("--dataset", type=str, default=r"dataset\HaluEval_qa.json")
